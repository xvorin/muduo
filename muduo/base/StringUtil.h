...
#ifndef MUDUO_BASE_STRING_UTIL_H_
#define MUDUO_BASE_STRING_UTIL_H_
...
// struct string_cmp_nocase : public std::function<std::string, std::string, bool> {
// public:
...
//         return (lhs.size() == rhs.size()) ? 0 : (lhs.size() < rhs.size()) ? 1
//                                                                           : 0;
//     }
// };
...
inline typename SequenceSequenceT::value_type join_if(const SequenceSequenceT& Input, const Range1T& Separator,
    PredicateT Pred)
{
    typedef typename SequenceSequenceT::value_type ResultT;
    typedef typename SequenceSequenceT::const_iterator InputIteratorT;
